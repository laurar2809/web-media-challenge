# Projekttagebuch

## KW 38 (15.09. - 21.09.2025)
 
### ABA Portal

 - Fertigstellung des ABA Portals
 - Einreichung abgeschlossen

### Website Prototyp mit TablerIO

  - Erstellung zweiter Website Prototype mithilfe von Tabler.Io
  - Klare Seitenstruktur (Navigation, Startseite, Unterseiten)
  - Erste visuelle Umsetzung des geplanten Designs

### Eigene Designs:

 > ![alt text](Images/image-1.png)

 > ![alt text](Images/image-2.png)

 ### TablerIO Inspirationen:

 > ![alt text](Images/image-3.png)

 > ![alt text](Images/image-4.png)

 > ![alt text](Images/image-5.png) 
 

### Probleme

 - Die Auswahl von Farben, Designs und Icons fiel mir nicht immer leicht. 

 ### Quellen

  - https://tabler.io/ 
  - https://fonts.google.com/icons
  - https://www.pexels.com/de-de/

## KW 39 (22.09. – 28.09.2025)
**Stunden gesamt: 3 h**

### Weiterführung zweiter Website-Prototyp
- Weiterarbeiten am zweiten Website-Prototypen  
- Detailliertere Ausarbeitung des Designs  
- Feinschliff an Layout und Struktur


## KW 40 (29.09 - 05.10.2025)

### Fertigstellung zweiter Website Prototype

 - Erweiterung der Teams und Challenge Seiten

 > ![alt text](Images/imageChallenges.png)

 > ![alt text](Images/imageChallenges01.png)

 > ![alt text](Images/imageTeams.png)

### Brainstorming der Designideen

 - Austausch und Diskussion verschiedener Designansätze mit Herrn Hanl
 - Recherche und Inspiration durch Bootstrap und Tabler.IO

 > ![alt text](Images/imageBootstrap01.png)

 > ![alt text](Images/imageBootstrap02.png)

 > ![alt text](Images/imageBootstrap03.png)

### Start mit drittem Website Prototypen

 - Entwicklung erster Scribbles für Kategorien und Challenges
 - Start mit Website Prototyp 03 auf XD mithilfe von Bootstrap Inspirationen

 > ![alt text](Images/imageXD.png)

 > ![alt text](Images/imageXD1.png)

 > ![alt text](Images/imageXD2.png)

### Quellen

 - https://tabler.io/ 
 - https://getbootstrap.com/
 - https://www.pexels.com/de-de/

## KW 41 (06.10. 12.10.2025)
 
### Weiterarbeiten am dritten Website-Prototypen auf XD

 - Erstellen der Seiten (Bearbeiten, Detail, Neu,...)
 - Verbindungen zwischen den einzelnen Seiten erstellen

 > ![alt text](Images/proto1.png)

 > ![alt text](Images/proto2.png)

 > ![alt text](Images/proto3.png)

 > ![alt text](Images/proto4.png)

 > ![alt text](Images/proto5.png)

 > ![alt text](Images/verbindungen.png)

 ### Einarbeiten in vorgegeben Code und Vorbereitung auf erstmaliges Programmieren

 - Erklärung von Herrn Hanl
 - Einarbeiten in den Code - erste Versuche
 - Änderung der Icons

 > ![alt text](Images/ordnerStruktur.png)

 > ![alt text](Images/code1.png)

 > ![alt text](Images/code2.png)

 > ![alt text](Images/icons.png)

 ### Quellen

 - https://getbootstrap.com/
 - https://www.pexels.com/de-de/
 - https://tabler.io/ 

 ## KW 42 (13.10. - 19.10.2025)

 ### Weiterarbeit der Frontentarbeiten auf der Website

 - Weiterführung der Frontend Arbeiten
 - Speichern u. Abbrechen Buttons
 - Dropdownbutton für Kategorien


### Details und Bearbeiten Links hinzufügen

### Quellen

 - https://getbootstrap.com/

## KW 43 (20.10. - 26.10.2025)
### Versuch die Detail Seite funktionell zu machen

 - Probleme bei der Programmierung
 - Fehlersuche

### Designideen recherieren 

 - Bootstrap Ideen sammeln

### Quellen

 - https://getbootstrap.com/
 - https://chat.deepseek.com/

## KW 44 (27.10. - 02.11.2025)

### Eigene Branch erstellen

### Beginn vom Design der Challenge Seite 

- Schriften und Buttons zentrieren
- Recherche auf Bootstrap

### Quellen

- https://getbootstrap.com/
- https://chat.deepseek.com/


## KW 45 (03.11. - 09.11.2025)

### Farbanpassung auf der Website

 - Anpassung der Farben auf Menüleisten und Buttons
 - Vereinheitlichung des Farbschemas für bessere Benutzerfreundlichkeit
 - Verbesserung der Kontraste für bessere Lesbarkeit 

 > ![alt text](Images/Menueleiste.png) 

 > ![alt text](Images/CodeNavbar.png)

### "Foto hochladen" Funktion

 - Hochgeladene Fotos werden nun korrekt gespeichert
 - Anzeige der Bilder innerhalb der Website funktioniert

 ![alt text](Images/FotosHochladen.png)

### Kleinere funktionelle Dinge

 - Kategorienamen korrekt anzeigen
 - Dateinamen sauber speichern
 - Entfernen unnötiger Filterfenster
 - Allgemeine Bugfixes und Aufräumarbeiten
 

## KW 46 (10.11. – 16.11.2025)
**Stunden gesamt: 10 h**

### Verbesserung der Challenge-Seite
- Überarbeitung der Challenge-Seite  
- Aufgabenpakete übersichtlicher gestalten  
- Darstellung mithilfe von Bootstrap Cards  

> ![alt text](Images/ChallengesCards.png)

### Design-Optimierungen
- Einheitlicher Aufbau der einzelnen Challenges  
- Bessere visuelle Trennung der Inhalte

### Quellen

- https://getbootstrap.com/
- https://chat.deepseek.com/



## KW 47 (17.11.2025 - 23.11.2025)
**Stunden gesamt: 0 h**

- Donnerstag: Fahrschule - Perfektionsfahrt



## KW 48 (24.11. – 30.11.2025)
**Stunden gesamt: 28 h**

### Verbesserung des Teams-Managements
- Überarbeitung des Teams-Managements  
- Einführung von Modal-Fenstern  
- Platzsparenderes und benutzerfreundlicheres Design 

> ![alt text](Images/Teamsmanagement.png) 

> ![alt text](Images/Teamsanzeige.png)

### Funktionalität des Teams-Managements
- Buttons funktionsfähig gemacht  
- Teams korrekt anzeigen  
- Graues Overlay entfernt  

### Allgemeine Designänderungen
- Designanpassungen auf mehreren Seiten  
- Loginfenster neu gestaltet  
- Buttons mit Icons ergänzt 

> ![alt text](Images/StyleLogin.png) 

### Projektmanagement
- Aktualisierung der Zeitaufzeichnung  
- Pflege des Projekttagebuchs  
- Überprüfung der Projektstruktur


## KW 49 (01.12. – 07.12.2025)
**Stunden gesamt: 17 h**

### Designänderungen der Challenge-Seite
- Designüberarbeitung der Challenge-Seite  
- Kleine funktionale Anpassungen  

### Design der Teams-Seite
- Layout-Verbesserungen  
- Anpassung von Farben und Abständen  
- Einheitlicher Stil mit restlicher Website


## KW 50 (08.12. – 14.12.2025)
**Stunden gesamt: 10 h**

### Teams-erstellen-Seite
- Umsetzung der Funktionalitäten 
- Design der Teams-erstellen-Seite


## KW 51 (15.12. – 21.12.2025)
**Stunden gesamt: 0 h**

- Donnerstag: Freier Tag


## KW 52 (22.12. – 28.12.2025)
**Stunden gesamt: 0 h**

- Ferien


## KW 01 (29.12.2025 – 04.01.2026)
**Stunden gesamt: 2 h**

### Besprechung mit Mirjam
- Austausch über weiteres Vorgehen  
- Diskussion über Partials  
- Besprechung der Code-Struktur  
- Besprechung ob älter Version nötig ist


## KW 02 (05.01. – 11.01.2026)
**Stunden gesamt: 15 h**

### Code-Strukturierung
- Umstrukturierung des Codes  
- Inline-Styles aus Layout und Login entfernt  
- Styles vollständig in CSS ausgelagert 

> ![alt text](Images/csscode1.png)

> ![alt text](Images/HtmlcodeLayout.png)

### Code-Optimierung
- Kommentare ergänzt  
- Doppelte Code-Teile entfernt  
- Unnötigen Code gelöscht  

> ![alt text](Images/Kommentare.png)


## KW 03 (12.01. - 18.01.2026)
**Stunden gesamt: 15 h**

### Code-Umstrukturierung
- Weitere Umstrukturierung des Codes (Entfernen der Inlinestyles, Strukturierung des CSS-Files)  
- Einheitliche Styles und bessere Lesbarkeit  
- Letzte Anpassungen vor Design-Feinschliff 

> ![alt text](Images/cssButton.png) 

> ![alt text](Images/cssNavigation.png)

### Design-Feinschliff
- Kleine Designänderungen auf der Website  
- Anpassungen bei Cards, Navbar, Abständen  
- Vereinheitlichung der Buttons für gleiche Funktionen 

> ![alt text](Images/Aufgabenpakete.png)


## KW 04 (19.01. - 25.01.2026)
**Stunden gesamt: 8 h**

### Design-Abgabeseiten (Lehreransicht)
- Bearbeiten des Designs der Abgabeseiten (Lehreransicht)
- Veränderungen an Abständen, Buttons und Statusanzeigen 

> ![alt text](Images/AbgabeseiteStatus.png) 

> ![alt text](Images/AbgabeDetailLehrer.png)

### Fertigstellung der Designänderungen
- Abschluss der Designänderungen der Abgabenseiten (Lehreransicht)
- Statusansichten klar und übersichtlich gestaltet  
- Übereinstimmung der Designs mit restlichem Website-Layout 
  
  